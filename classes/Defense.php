                ...
                if ($other_secdef_present !== false && $qty > 0)
                {
                    foreach ($other_secdef_present as $tmp_other_defense)
                    {
                        ...
                        if ($qty > $tmp_other_defense['quantity'])
                        {
                           ...
                           Tki\Models\PlayerLog::writeLog($pdo_db, $tmp_other_defense['ship_id'], LogEnums::DEFS_DESTROYED, $tmp_other_defense['quantity'] . '|' . $targetdeftype . '|' . $tmp_defense['sector_id']);
                           Tki\Models\PlayerLog::writeLog($pdo_db, $tmp_defense['ship_id'], LogEnums::DEFS_DESTROYED, $tmp_other_defense['quantity'] . '|' . $deftype . '|' . $tmp_defense['sector_id']);
                        }
                        else
                        {
                            $sql = "DELETE FROM ::prefix::sector_defense WHERE defense_id = :defense_id";
                           ...
                           Tki\Models\PlayerLog::writeLog($pdo_db, $tmp_other_defense['ship_id'], LogEnums::DEFS_DESTROYED, $qty . '|' . $targetdeftype . '|' . $tmp_defense['sector_id']);
                           Tki\Models\PlayerLog::writeLog($pdo_db, $tmp_defense['ship_id'], LogEnums::DEFS_DESTROYED, $qty . '|' . $deftype . '|' . $tmp_defense['sector_id']);
                            $qty = 0;
                        }
                    }
                }
