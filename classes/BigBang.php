...
/**
 * classes/BigBang.php from The Kabal Invasion.
...
class BigBang
{
    public function findStep(string $current_file): array
    {
        ...
        // We add a null value as index 0 due to the need to start from index 1.
        ...
        if ($current_file === '')
        {
            ...
            $bigbang_info['current_step'] = (int) array_search('0.php', $bigbang_files, true);
        }
        else
        {
            // Usual search, from the current step
            $bigbang_info['current_step'] = (int) array_search(basename($current_file), $bigbang_files, true);
        }
