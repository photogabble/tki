...
class Ibank
{
    public static function ibankBorrow(\PDO $pdo_db, string $lang, Registry $tkireg, Timer $tkitimer, array $playerinfo, array $account, int $amount, Smarty $template): void
    {
        ...
        if (($amount * 1) != $amount)
        {
            self::ibankError($pdo_db, $lang, $langvars['l_ibank_invalidamount'], "ibank.php?command=loans", $tkireg, $tkitimer, $template);
        }
        ...
        if ($amount > $maxtrans)
        {
            self::ibankError($pdo_db, $lang, $langvars['l_ibank_loantoobig'], "ibank.php?command=loans", $tkireg, $tkitimer, $template);
        }
    ...
    public static function ibankRepay(\PDO $pdo_db, string $lang, array $playerinfo, array $account, int $amount, Registry $tkireg, Timer $tkitimer, Smarty $template): void
    {
        ...
        if ($account['loan'] == 0)
        {
            self::ibankError($pdo_db, $lang, $langvars['l_ibank_notrepay'], "ibank.php?command=loans", $tkireg, $tkitimer, $template);
        }
        ...
        {
            self::ibankError($pdo_db, $lang, $langvars['l_ibank_notenoughrepay'], "ibank.php?command=loans", $tkireg, $tkitimer, $template);
        }
