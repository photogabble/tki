...
// TODO: Refactor all usages so Registry is no longer required
// TODO: Refactor $ownerinfo to be Ship/User/etc
// TODO: Refactor $planetinfo to be Planet
...
class CalcLevels
{
    ...
    {
        $base_factor = ($planetinfo['base'] == 'Y') ? config('game.base_defense') : 0;
        ...
        if ($beam_defender_here !== false)
        {
            ...
            {
                $planetbeams = $planetbeams + self::abstractLevels($tmp_beams['beams'], $tkireg);
            }
        }
    ...
    public static function planetShields(\PDO $pdo_db, array $ownerinfo, Registry $tkireg, array $planetinfo): int
    {
        ...
        if ($shield_defender_here !== false)
        {
            ...
            {
                $planetshields = $planetshields + self::abstractLevels($tmp_shields['shields'], $tkireg);
            }
        }
    ...
    {
        $base_factor = ($planetinfo['base'] == 'Y') ? config('game.base_defense') : 0;
        $torp_launchers = round(pow(config('game.level_factor'), ($ownerinfo['torp_launchers']) + $base_factor)) * 10;
        ...
        if ($torp_defender_here !== false)
        {
            ...
            {
                $ship_torps = round(pow(config('game.level_factor'), $tmp_torp['torp_launchers'])) * 10;
