...
namespace Tki\Actions\TradeRoutes;
    ...
    public static function engage(\PDO $pdo_db, string $lang, int $tr_repeat, Registry $tkireg, Timer $tkitimer, array $playerinfo, int $engage, array $traderoutes, ?int $portfull, Smarty $template): void
    {
        ...
        if ($traderoute['source_type'] == 'P')
        {
            ...
            if ($source['port_type'] == 'special')
            {
                ...
                {
                    $free_fighters = Helpers\CalcLevels::abstractLevels($playerinfo['computer'], $tkireg) - $playerinfo['ship_fighters'];
                ...
                {
                    $free_torps = Helpers\CalcLevels::abstractLevels($playerinfo['torp_launchers'], $tkireg) - $playerinfo['torps'];
            ...
            else
            {
                ...
                {
                    $energy_price = $energy_price - $tkireg->energy_delta * $source['port_energy'] / $tkireg->energy_limit * $tkireg->inventory_factor;
                    $energy_buy = Helpers\CalcLevels::energy($playerinfo['power'], $tkireg) - $playerinfo['ship_energy'] - $dist['scooped1'];
                ...
                if ($dist['scooped1'] > 0)
                {
                    ...
                    if ((int) $playerinfo['ship_energy'] > Helpers\CalcLevels::energy($playerinfo['power'], $tkireg))
                    {
                        $playerinfo['ship_energy'] = Helpers\CalcLevels::energy($playerinfo['power'], $tkireg);
                    }
                }
