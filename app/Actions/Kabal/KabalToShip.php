    ...
    public static function ship(\PDO $pdo_db, string $lang, int $ship_id, Registry $tkireg, array $playerinfo): void
    {
        ...
        if (!$attackerarmor > 0)
        {
            ...
            if ($targetarmor > 0)
            {
                ...
                $free_holds = Helpers\CalcLevels::abstractLevels($targetinfo['hull'], $tkireg) - $targetinfo['ship_ore'] - $targetinfo['ship_organics'] - $targetinfo['ship_goods'] - $targetinfo['ship_colonists'];
                $salv_goods = 0;
                if ($free_holds > $free_goods)
