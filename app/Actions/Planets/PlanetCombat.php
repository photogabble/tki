...
class PlanetCombat
{
    public static function prime(\PDO $pdo_db, string $lang, Registry $tkireg, Timer $tkitimer, Smarty $template, array $playerinfo, array $ownerinfo, array $planetinfo): bool
    {
        ...
        $planetbeams        = Helpers\CalcLevels::planetBeams($pdo_db, $ownerinfo, $tkireg, $planetinfo);
        $planetfighters     = $planetinfo['fighters'];
        $planetshields      = Helpers\CalcLevels::planetShields($pdo_db, $ownerinfo, $tkireg, $planetinfo);
        $planettorps        = Helpers\CalcLevels::planetTorps($pdo_db, $ownerinfo, $planetinfo, $tkireg);
        ...
        if (is_array($shipsOnPlanet))
        {
            ...
            while ($shipsOnPlanetCount > 0)
            {
                ...
                Actions\Combat::shipToShip($pdo_db, $lang, $onplanet['ship_id'], $tkireg, $playerinfo, $attackerbeams, $attackerfighters, $attackershields, $attackertorps, $attackerarmor, $attackertorpdamage);
                $shipsOnPlanetCount--;
            }
        }
        else
        ...
        if ($attackerarmor < 1)
        {
            ...
            if ($playerinfo['dev_escapepod'] == "Y")
            {
                ...
                // $planet->owner->bounties->each(function(\Tki\Models\Bounty $bounty){
                //     $bounty->collect($player);
                // });
                // Actions\Bounty::collect($pdo_db, $lang, $planetinfo['owner'], $playerinfo['ship_id']);
            }
            else
            {
        ...
        if ($planetshields < 1 && $planetfighters < 1 && $attackerarmor > 0 && $shipsonplanet == 0)
        {
            ...
            $self_tech = Helpers\CalcLevels::avgTech($playerinfo);
            $target_tech = round(Helpers\CalcLevels::avgTech($ownerinfo));
            ...
            {
                $playerscore = Actions\Score::updateScore($pdo_db, $playerinfo['ship_id'], $tkireg, $playerinfo);
                ...
                if ($playerscore < $planetscore)
                {
                    ...
                    \Tki\Models\PlayerLog::writeLog($pdo_db, $ownerinfo['ship_id'], LogEnums::PLANET_DEFEATED_D, "$planetinfo[name]|$playerinfo[sector]|$playerinfo[character_name]");
                    ...
                    Actions\Score::updateScore($pdo_db, $ownerinfo['ship_id'], $tkireg, $playerinfo);
                }
                else
                {
                    ...
                    \Tki\Models\PlayerLog::writeLog($pdo_db, $ownerinfo['ship_id'], LogEnums::PLANET_DEFEATED, "$planetinfo[name]|$playerinfo[sector]|$playerinfo[character_name]");
                    Actions\Score::updateScore($pdo_db, $ownerinfo['ship_id'], $tkireg, $playerinfo);
        ...
        else
        {
            ...
            \Tki\Models\PlayerLog::writeLog($pdo_db, $ownerinfo['ship_id'], LogEnums::PLANET_NOT_DEFEATED, "$planetinfo[name]|$playerinfo[sector]|$playerinfo[character_name]|$free_ore|$free_organics|$free_goods|$ship_salvage_rate|$ship_salvage");
            Actions\Score::updateScore($pdo_db, $ownerinfo['ship_id'], $tkireg, $playerinfo);
