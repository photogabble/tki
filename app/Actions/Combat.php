...
/**
 * classes/Combat.php from The Kabal Invasion.
...
class Combat
{
    public static function shipToShip(\PDO $pdo_db, string $lang, int $ship_id, Registry $tkireg, array $playerinfo, int $attackerbeams, int $attackerfighters, int $attackershields, int $attackertorps, int $attackerarmor, int $attackertorpdamage): void
    {
        ...
        $targetbeams = Helpers\CalcLevels::abstractLevels($targetinfo['beams'], $tkireg);
        ...
        $targetshields = Helpers\CalcLevels::abstractLevels($targetinfo['shields'], $tkireg);
        ...
        if ($targetarmor < 1)
        {
            ...
            if ($attackerarmor > 0)
            {
                ...
                $free_holds = Helpers\CalcLevels::abstractLevels($playerinfo['hull'], $tkireg) - $playerinfo['ship_ore'] - $playerinfo['ship_organics'] - $playerinfo['ship_goods'] - $playerinfo['ship_colonists'];
                $salv_goods = 0;
            ...
            if ($targetinfo['dev_escapepod'] == "Y")
            {
                ...
                \Tki\Models\PlayerLog::writeLog($pdo_db, $targetinfo['ship_id'], LogEnums::ATTACK_LOSE, "$playerinfo[character_name]|Y");
                Actions\Bounty::collect($pdo_db, $lang, $playerinfo['ship_id'], $targetinfo['ship_id']);
            }
            else
            {
                \Tki\Models\PlayerLog::writeLog($pdo_db, $targetinfo['ship_id'], LogEnums::ATTACK_LOSE, "$playerinfo[character_name]|N");
        ...
        else
        {
            ...
            \Tki\Models\PlayerLog::writeLog($pdo_db, $targetinfo['ship_id'], LogEnums::ATTACKED_WIN, "$playerinfo[character_name]|$target_armor_lost|$target_fighters_lost");
